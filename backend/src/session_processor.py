        ...
        def __iter__(self):
            for item in self.iterable:
                ...
                if self.total and self.count % max(1, self.total // 20) == 0:
                    ...
                    print(
                        f"\r{self.desc}: {self.count}/{self.total} ({percent:.1f}%)",
                        end="",
                        flush=True,
                    )
            if self.total:
...
class SessionProcessor:
    ...
    def should_include_session(self, session: Dict, filters: Dict) -> bool:
        ...
        if "sports" in filters:
            session_sports = [
                exercise.get("sport") for exercise in session.get("exercises", [])
            ]
    ...
    def extract_metrics(self, session: Dict) -> Dict:
        ...
        formatted_durations = [
            str(timedelta(seconds=seconds)) for seconds in autolap_durations
        ]

        # Store durations as semicolon-delimited string for CSV compatibility
        metrics["autolap_durations"] = (
            ";".join(formatted_durations) if formatted_durations else ""
        )

        return metrics

    def process_sessions(
        self, filters: Dict = None, output_file: str = "chart_data.json"
    ):
    ...
    def export_to_chartjs(self, output_file: str):
        ...
        chart_config = {
            ...
            "options": {
                "responsive": True,
                "plugins": {
                    "title": {"display": True, "text": "Autolap Times by Session"},
                    "legend": {
    ...
    def register_chart(self, output_file: str, datasets: List[Dict]):
        ...
        distances = [
            d.get("meta", {}).get("distance", 0)
            for d in datasets
            if d.get("meta", {}).get("distance", 0) > 0
        ]
...
def main():
    ...
    parser.add_argument(
        "--output", default="chart_data.json", help="Output Chart.js JSON file"
    )
    parser.add_argument(
        "--sport",
        action="append",
        help="Filter by sport type (can be used multiple times)",
    )
